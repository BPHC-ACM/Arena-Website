...
export function ThrowballForm({ match, onSave, isCreate }: any) {
  const [form, setForm] = useState(match ?? DEFAULTS['throwball']);
  ...
  const winPt = (team: 'A' | 'B') => {
    ...
    const minPts = 15;
    const isSetWon = (pts >= minPts && pts - opp >= 2) || pts === 21; // Assuming 21 pt cap
  ...
  return (
    <div className='space-y-5'>
      ...
      <StatusSelect sport='throwball' value={form.status ?? ''} onChange={handleStatusChange} />
